...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await callEcho(`/Shipments/${shipmentId}`);
    const echoResponse = await readEchoBody(response);

    if (!response.ok) {
      return NextResponse.json(
        { ok: false, error: "Echo shipment status request failed.", status: response.status, statusText: response.statusText, echoResponse },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unable to fetch Echo shipment status." },
      { status: 400 },
    );
  }
}
