...
function getProductTotalCents(quantity: number) { const subtotal = quantity * RETAIL_UNIT_AMOUNT; const discount = Math.round(subtotal * getDiscountRate(quantity)); return subtotal - discount; }
...
async function createPendingOrder(input: { supabase: ReturnType<typeof getSupabaseAdminClient>; customerId: string; body: CustomerCheckoutBody; quantity: number }) {
  ...
  const { error: activityError } = await input.supabase.from("crm_activity").insert({ activity_type: "checkout_started", title: `Customer checkout started for order ${orderId}`, description: `${clean(input.body.email)} started checkout for ${input.quantity} CowStop form(s).`, order_id: orderId, source: "customer_checkout", status: "open" });
