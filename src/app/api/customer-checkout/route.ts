...
const RETAIL_UNIT_PRICE = 1499;
const RETAIL_UNIT_AMOUNT = 149900;
...
function getQuantity(body: CustomerCheckoutBody) {
...
function getProductTotalCents(quantity: number) {
  const subtotal = quantity * RETAIL_UNIT_AMOUNT;
  const discount = Math.round(subtotal * getDiscountRate(quantity));
  return subtotal - discount;
}

function getFreightChargeCents(body: CustomerCheckoutBody) {
  const freightCharge = Number(body.freightCharge ?? 0);
  if (!Number.isFinite(freightCharge) || freightCharge <= 0) {
    throw new Error("Selected freight charge is required before checkout.");
  }
  return Math.round(freightCharge * 100);
}

function validateBody(body: CustomerCheckoutBody) {
  const quantity = getQuantity(body);

  if (!clean(body.email).includes("@")) {
    throw new Error("A valid order contact email is required.");
  }

  if (clean(body.phone).replace(/[^0-9]/g, "").length < 10) {
    throw new Error("A valid delivery contact phone is required.");
  }
  ...
  const required = [
    ...
    body.shipToZip,
    body.deliveryType,
    body.liftgateRequired,
    body.selectedRate,
  ];

  if (required.some((value) => !clean(value))) {
    throw new Error("Shipping details, delivery details, and selected freight option are required.");
  }
...
async function upsertCustomer(input: { supabase: ReturnType<typeof getSupabaseAdminClient>; body: CustomerCheckoutBody }) {
  ...
  const customerData = {
    email,
    first_name: clean(input.body.firstName) || null,
    last_name: clean(input.body.lastName) || null,
    phone: clean(input.body.phone) || null,
    company: clean(input.body.company) || null,
...
async function createPendingOrder(input: {
  ...
  const notes = [
    clean(input.body.notes),
    `Selected freight: ${clean(input.body.selectedRate)}`,
    `Delivery type: ${clean(input.body.deliveryType)}`,
    `Liftgate required: ${clean(input.body.liftgateRequired)}`,
    `Freight & handling: $${freightCharge.toFixed(2)}`,
  ].filter(Boolean).join("\n");
  ...
  if (error || !data?.id) {
    throw new Error(`Unable to create customer order before checkout: ${error?.message || "missing order id"}`);
  }
  ...
  const { error: activityError } = await input.supabase.from("crm_activity").insert({
    ...
    title: `Customer checkout started for order ${orderId}`,
    description: `${clean(input.body.email)} started checkout for ${input.quantity} CowStop form(s).`,
    order_id: orderId,
  ...
  if (activityError) {
    console.warn("Unable to create customer checkout CRM activity", activityError.message);
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const orderId = await createPendingOrder({ supabase, customerId, body, quantity });

    const productTotalCents = getProductTotalCents(quantity);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          ...
          price_data: {
            currency: "usd",
            unit_amount: productTotalCents,
            product_data: {
              name: "CowStop Reusable Form",
        ...
        },
        {
          ...
          price_data: {
            currency: "usd",
            unit_amount: freightChargeCents,
            product_data: {
      ...
      metadata: {
        ...
        product_total: String(productTotalCents / 100),
        freight_charge: String(freightChargeCents / 100),
        selected_rate: clean(body.selectedRate),
        contact_phone: clean(body.phone),
        delivery_type: clean(body.deliveryType),
        liftgate_required: clean(body.liftgateRequired),
        ship_to_name: clean(body.shipToName),
        ship_to_address: clean(body.shipToAddress),
        ship_to_city: clean(body.shipToCity),
        ship_to_state: clean(body.shipToState),
        ship_to_zip: clean(body.shipToZip),
      },
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to start customer checkout right now." },
      { status: 400 },
    );
  }
}
