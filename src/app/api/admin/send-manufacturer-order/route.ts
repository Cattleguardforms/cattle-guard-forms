...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!shipmentId) throw new Error("Echo shipment/load ID was not found on this order. Book the shipment before emailing the manufacturer.");

    const bolAttachment = await fetchEchoBolAttachment(shipmentId, clean(orderRecord.bol_number));
    const template = buildManufacturerFulfillmentTemplate(buildOrderPayload(orderRecord, bolAttachment.filename));
    const resend = new Resend(process.env.RESEND_API_KEY);
    const recipients = emails(process.env.MANUFACTURER_EMAILS);
    if (!process.env.RESEND_API_KEY) throw new Error("RESEND_API_KEY is required to send manufacturer email.");
    if (recipients.length === 0) throw new Error("MANUFACTURER_EMAILS must include at least one recipient.");

    const result = await resend.emails.send({
      from: process.env.FROM_EMAIL || "orders@cattleguardforms.com",
      to: recipients,
      replyTo: process.env.REPLY_TO_EMAIL || process.env.ORDERS_EMAIL || "orders@cattleguardforms.com",
    ...
    await supabase.from("orders").update({ manufacturer_notes: [previousNotes, `Manufacturer email sent: ${new Date().toISOString()} with ${bolAttachment.filename}`].filter(Boolean).join("\n"), updated_at: new Date().toISOString() }).eq("id", orderId);
    return NextResponse.json({ ok: true, orderId, shipmentId, bolFileName: bolAttachment.filename, recipients, result });
  } catch (error) {
