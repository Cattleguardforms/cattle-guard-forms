...
type BookingPayload = { ok?: boolean; error?: string; bolNumber?: string; echoLoadId?: string; echoResponse?: unknown; shipmentRequest?: unknown; status?: number; statusText?: string };
...
function quantity(order: AdminOrder) {
  return order.quantity_display ?? order.cowstop_quantity ?? order.quantity ?? 0;
}
  ...
  async function executeShipment(orderId: string, dryRunMode: boolean) {
    ...
    try {
      ...
      const response = await fetch("/api/echo/book-ltl-shipment", {
        ...
        body: JSON.stringify({ orderId, dryRun: dryRunMode }),
      });
