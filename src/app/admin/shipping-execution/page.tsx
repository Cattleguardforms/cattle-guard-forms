...
type BookingPayload = { ok?: boolean; error?: string; bolNumber?: string; echoLoadId?: string; echoResponse?: unknown; shipmentRequest?: unknown; status?: number; statusText?: string; attemptedPaths?: unknown };
...
function filenameFromDisposition(disposition: string, fallback: string) { const match = disposition.match(/filename\*=UTF-8''([^;]+)|filename="?([^";]+)"?/i); return decodeURIComponent(match?.[1] || match?.[2] || fallback); }
function extensionFromContentType(contentType: string) { const lower = contentType.toLowerCase(); if (lower.includes("pdf")) return "pdf"; if (lower.includes("jpeg") || lower.includes("jpg")) return "jpg"; if (lower.includes("png")) return "png"; return "bin"; }
function isDownloadableBol(contentType: string) { const lower = contentType.toLowerCase(); return lower.includes("application/pdf") || lower.includes("image/jpeg") || lower.includes("image/jpg") || lower.includes("image/png") || lower.includes("application/octet-stream"); }
...
export default function AdminShippingExecutionPage() {
  ...
  const [lastResult, setLastResult] = useState<BookingPayload | null>(null);
  ...
  async function executeShipment(orderId: string, dryRunMode: boolean) {
    ...
    try {
      ...
      setNotice(dryRunMode ? "Echo dry run built successfully. Review the request below before booking." : `Echo shipment booked. BOL ${payload.bolNumber || "created"}.`);
      await loadOrders();
  ...
  async function fetchBol(orderId: string) {
    ...
    try {
      const token = await getToken();
      const response = await fetch("/api/echo/bol", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ orderId }) });
      const contentType = response.headers.get("content-type") || "";
      if (response.ok && isDownloadableBol(contentType)) {
        const blob = await response.blob();
        const extension = extensionFromContentType(contentType);
        const fallback = `Echo-BOL-${shortId(orderId)}.${extension}`;
        ...
        setNotice(`Downloaded Echo BOL: ${link.download}`);
        await loadOrders();
        return;
      }
      const text = await response.text();
      try {
        const payload = JSON.parse(text) as BookingPayload;
        setLastResult(payload);
        throw new Error(payload.error || "Echo BOL download failed.");
      } catch {
        throw new Error(`Echo BOL download failed. Unexpected response: ${text.slice(0, 300)}`);
      }
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to fetch Echo BOL."); }
    finally { setFetchingBol(false); }
  }

  return <main className="min-h-screen bg-neutral-50 text-neutral-950"><header className="border-b border-neutral-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5"><Link href="/admin" className="font-semibold text-green-800">Admin Portal</Link><nav className="flex items-center gap-6 text-sm font-medium text-neutral-700"><Link href="/admin/orders" className="hover:text-green-800">Orders</Link><Link href="/admin/shipping-execution" className="text-green-900 underline decoration-green-800 decoration-2 underline-offset-8">Ship / Execute</Link><Link href="/admin/distributors" className="hover:text-green-800">Distributors</Link></nav></div></header><section className="mx-auto max-w-7xl px-6 py-12"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-sm font-semibold uppercase tracking-wide text-green-800">Admin / Shipping execution</p><h1 className="mt-3 text-4xl font-bold tracking-tight">Book Echo Shipment / Download BOL</h1><p className="mt-4 max-w-3xl leading-8 text-neutral-700">Execute paid orders, book Echo LTL shipments, and fetch the Echo BOL without manually typing every shipment field.</p></div><button onClick={() => void loadOrders()} className="rounded-lg border border-neutral-300 bg-white px-4 py-2 text-sm font-semibold hover:bg-neutral-50">Refresh Orders</button></div>{error ? <div className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{error}</div> : null}{notice ? <div className="mt-6 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-900">{notice}</div> : null}<div className="mt-8 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]"><section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Paid / Ready Orders</h2><p className="mt-2 text-sm text-neutral-600">Select an order to book or download its BOL.</p><div className="mt-5 max-h-[620px] space-y-3 overflow-auto pr-2">{loading ? <p className="text-sm text-neutral-600">Loading orders...</p> : null}{!loading && executableOrders.length === 0 ? <p className="text-sm text-neutral-600">No executable paid orders found.</p> : null}{executableOrders.map((order) => <button key={order.id} type="button" onClick={() => { setSelectedId(order.id); setLastResult(null); setError(""); setNotice(""); }} className={selectedId === order.id ? "w-full rounded-xl border border-green-700 bg-green-50 p-4 text-left" : "w-full rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-left hover:border-green-700 hover:bg-green-50"}><div className="flex items-start justify-between gap-3"><div><p className="font-black text-green-950">Order {shortId(order.id)}</p><p className="mt-1 text-sm text-neutral-700">{statusText(order.customer_display_name)}</p><p className="mt-1 text-xs text-neutral-500">{quantity(order)} form(s) - {money(order.amount_display ?? order.amount_paid ?? order.total, order.currency ?? "USD")}</p></div><span className="rounded bg-white px-2 py-1 text-xs font-bold text-neutral-700 ring-1 ring-neutral-200">{statusText(order.shipment_status)}</span></div><p className="mt-2 text-xs text-neutral-500">Carrier: {statusText(order.carrier)} | BOL: {statusText(order.bol_number)}</p></button>)}</div></section><section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">{!selectedOrder ? <div><h2 className="text-xl font-black">Select an order</h2><p className="mt-2 text-sm text-neutral-600">Choose an order from the left to review and execute shipping.</p></div> : <div><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-green-800">Selected order</p><h2 className="mt-2 text-2xl font-black">Order {shortId(selectedOrder.id)}</h2><p className="mt-2 text-sm text-neutral-600">{statusText(selectedOrder.customer_display_name)} - {statusText(selectedOrder.customer_email)}</p></div><Link href={`/admin/orders?order=${encodeURIComponent(selectedOrder.id)}`} className="rounded border border-neutral-300 bg-white px-4 py-2 text-sm font-bold text-neutral-800 hover:bg-neutral-50">Open order detail</Link></div><div className="mt-6 grid gap-3 md:grid-cols-2"><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">Quantity</p><p className="mt-1 font-bold">{quantity(selectedOrder)} form(s)</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">Payment</p><p className="mt-1 font-bold">{statusText(selectedOrder.payment_status || selectedOrder.checkout_status)}</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">Selected rate</p><p className="mt-1 text-sm font-bold">{statusText(selectedOrder.selected_rate)}</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">Freight charge</p><p className="mt-1 font-bold">{money(selectedOrder.freight_charge ?? 0)}</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">Current carrier</p><p className="mt-1 font-bold">{statusText(selectedOrder.carrier)}</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3"><p className="text-xs font-bold uppercase text-neutral-500">BOL</p><p className="mt-1 font-bold">{statusText(selectedOrder.bol_number)}</p></div><div className="rounded-xl border border-neutral-200 bg-neutral-50 p-3 md:col-span-2"><p className="text-xs font-bold uppercase text-neutral-500">Created</p><p className="mt-1 font-bold">{dateText(selectedOrder.created_at)}</p></div></div><div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><p className="font-bold">Execution rules</p><p className="mt-1">Use dry run first when checking a live order. After booking, use Fetch / Download Echo BOL. Echo may return the BOL as a PDF or image file.</p></div><div className="mt-5 flex flex-wrap gap-3"><button disabled={executing || fetchingBol} onClick={() => void executeShipment(selectedOrder.id, true)} className="rounded border border-green-800 bg-white px-5 py-3 text-sm font-bold text-green-900 hover:bg-green-50 disabled:opacity-50">Dry Run Echo Request</button><button disabled={executing || fetchingBol} onClick={() => void executeShipment(selectedOrder.id, false)} className="rounded bg-green-800 px-5 py-3 text-sm font-bold text-white hover:bg-green-900 disabled:opacity-50">{executing ? "Executing..." : "Book / Execute Shipment"}</button><button disabled={executing || fetchingBol || !selectedOrder.bol_number} onClick={() => void fetchBol(selectedOrder.id)} className="rounded bg-blue-800 px-5 py-3 text-sm font-bold text-white hover:bg-blue-900 disabled:opacity-50">{fetchingBol ? "Fetching BOL..." : "Fetch / Download Echo BOL"}</button></div>{lastResult ? <details className="mt-6 rounded-xl border border-neutral-200 bg-neutral-950 p-4 text-neutral-50" open><summary className="cursor-pointer text-sm font-bold">Echo result / request details</summary><pre className="mt-3 max-h-96 overflow-auto text-xs">{JSON.stringify(lastResult, null, 2)}</pre></details> : null}</div>}</section></div></section></main>;
}
