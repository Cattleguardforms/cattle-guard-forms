      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-neutral-600">Open an order to fetch or download the BOL, book shipment, open warranty paperwork, and download stored order files.</p>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.25fr]">
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-sm">
            <div className="border-b border-neutral-200 p-5">
              ...
              <p className="mt-1 text-sm text-neutral-600">Click an order to manage documents and freight actions.</p>
            </div>
                ...
                {orders.map((order) => (
                  <button key={order.id} type="button" onClick={() => void selectOrder(order.id)} className={`rounded-xl border p-4 text-left hover:border-green-800 hover:bg-green-50 ${selectedOrderId === order.id ? "border-green-800 bg-green-50" : "border-neutral-200 bg-neutral-50"}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-black text-green-950">Order {shortId(order.id)}</p>
                        <p className="mt-1 text-sm text-neutral-700">{text(order.customer_display_name)}</p>
                        <p className="mt-1 text-xs text-neutral-600">{text(order.customer_email)}</p>
                      </div>
                      <span className="rounded-full bg-white px-3 py-1 text-xs font-bold ring-1 ring-neutral-200">{text(order.shipment_status)}</span>
                    </div>
                    <p className="mt-2 text-xs text-neutral-600">Qty: {qty(order)} | Paid: {money(order.amount_display ?? order.amount_paid ?? order.total, order.currency ?? "USD")}</p>
                    <p className="mt-1 text-xs text-neutral-600">Carrier: {text(order.carrier)} | BOL: {text(order.bol_number)}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-sm">
            <div className="border-b border-neutral-200 p-5">
              ...
              <p className="mt-1 text-sm text-neutral-600">Customer, shipment, BOL, warranty, and document actions.</p>
            </div>
            {!selectedOrder ? (
              <div className="p-6 text-sm text-neutral-600">Select an order from the left.</div>
            ) : (
              <div className="p-5">
                <div className="grid gap-3 md:grid-cols-2">
                  <Detail label="Order" value={selectedOrder.id} />
                  <Detail label="Customer / Distributor" value={text(selectedOrder.customer_display_name)} />
                  <Detail label="Email" value={text(selectedOrder.customer_email)} />
                  <Detail label="Phone" value={text(selectedOrder.customer_phone)} />
                  <Detail label="Quantity" value={String(qty(selectedOrder))} />
                  <Detail label="Amount" value={money(selectedOrder.amount_display ?? selectedOrder.amount_paid ?? selectedOrder.total, selectedOrder.currency ?? "USD")} />
                  <Detail label="Payment" value={text(selectedOrder.payment_status)} />
                  <Detail label="Shipment" value={text(selectedOrder.shipment_status)} />
                  <Detail label="Carrier" value={text(selectedOrder.carrier)} />
                  <Detail label="BOL Number" value={text(selectedOrder.bol_number)} />
                  <Detail label="Created" value={dateText(selectedOrder.created_at)} />
                  <Detail label="Estimated Delivery" value={dateText(selectedOrder.estimated_delivery_date)} />
                </div>
                ...
                <div className="mt-6 rounded-xl border border-neutral-200 bg-neutral-50 p-4">
                  <h3 className="text-lg font-black">BOL Downloads</h3>
                  {filesLoading ? <p className="mt-2 text-sm text-neutral-600">Loading files...</p> : null}
                  {!filesLoading && bolFiles.length === 0 ? <p className="mt-2 text-sm text-neutral-600">No BOL file is stored yet. Click Fetch BOL above.</p> : null}
                  <div className="mt-3 grid gap-2">
                    {bolFiles.map((file) => <FileRow key={file.id} file={file} downloadingFileId={downloadingFileId} onDownload={downloadFile} primary />)}
                  </div>
                </div>

                <div className="mt-5 rounded-xl border border-neutral-200 bg-white p-4">
                  <h3 className="text-lg font-black">All Order Files</h3>
                  {!filesLoading && files.length === 0 ? <p className="mt-2 text-sm text-neutral-600">No stored order files yet.</p> : null}
                  <div className="mt-3 grid gap-2">
                    {otherFiles.map((file) => <FileRow key={file.id} file={file} downloadingFileId={downloadingFileId} onDownload={downloadFile} />)}
                  </div>
                </div>
              </div>
            )}
