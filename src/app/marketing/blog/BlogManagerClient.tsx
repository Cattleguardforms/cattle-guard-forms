...
function toPayload(record: BlogRecord, images: BlogImageSelection[]) {
  return {
    title: record.title,
...
export default function BlogManagerClient() {
  ...
  function updateField(field: keyof BlogRecord, value: string) {
    setForm((current) => {
      ...
      if (field === "title" && !editingId) { next.slug = slugify(value); next.seo_title = value; }
      return next;
    });
  }
  ...
  function addLibraryImage(id: string) {
    ...
    if (!item) return;
    setImages((current) => {
      if (current.some((image) => image.id === item.id)) return current;
      return [...current, { ...item, role: current.length === 0 ? "featured" : "body", placement: current.length === 0 ? "hero" : "middle" }];
    });
  }
  ...
  function updateImage(index: number, field: keyof BlogImageSelection, value: string) {
    setImages((current) => current.map((image, imageIndex) => {
      ...
      if (field === "role" && value === "featured") {
        return { ...image, role: "featured", placement: "hero" };
      }
      return { ...image, [field]: value } as BlogImageSelection;
    }).map((image, imageIndex, list) => {
      const featuredIndex = list.findIndex((entry) => entry.role === "featured");
      if (field === "role" && value === "featured" && imageIndex !== index && imageIndex !== featuredIndex) return { ...image, role: "body" };
      return image;
    }));
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (images.length < 4) { setError("Choose at least 4 blog images from the picture library before publishing or saving."); return; }
    setSaving(true);
    try {
      const response = await fetch(editingId ? `/api/marketing/blog-posts/${editingId}` : "/api/marketing/blog-posts", {
        method: editingId ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(toPayload(form, images)),
      });
      const data = await readJson<BlogApiResponse>(response);
  ...
  return (
    <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200">
        <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{editingId ? "Edit AI blog + image package" : "Create AI blog + image package"}</h2><p className="mt-2 text-sm leading-6 text-neutral-600">Write the blog, choose 3-4 relevant images from the picture library, preview it, then publish to the public blog.</p></div><button type="button" onClick={() => void loadPosts()} className="rounded border border-neutral-300 px-3 py-2 text-xs font-semibold hover:border-green-800 hover:bg-green-50">Refresh</button></div>
        <div className="mt-4 flex flex-wrap gap-3"><Link href="/marketing/ai" className="rounded bg-green-800 px-4 py-2 text-sm font-semibold text-white hover:bg-green-900">Generate Campaign Copy</Link><Link href="/blog" className="rounded border border-neutral-300 px-4 py-2 text-sm font-semibold hover:border-green-800 hover:bg-green-50">View Public Blog</Link></div>
