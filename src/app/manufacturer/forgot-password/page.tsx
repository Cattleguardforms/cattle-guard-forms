...
export default function ManufacturerForgotPasswordPage() {
  ...
  async function sendReset(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!normalizedEmail || !normalizedEmail.includes("@")) throw new Error("Enter a valid manufacturer email address.");
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(normalizedEmail, {
        redirectTo: `${siteUrl()}/auth/update-password?next=/manufacturer`,
      });
      if (resetError) throw new Error(resetError.message || "Unable to send password reset email.");
      setNotice("Password reset email sent. Check your inbox for the reset link, then return to the manufacturer portal to sign in.");
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-neutral-50 px-6 py-10 text-neutral-950">
      <section className="mx-auto max-w-xl rounded-2xl bg-white p-8 shadow-sm ring-1 ring-neutral-200">
        <Link href="/manufacturer" className="text-sm font-semibold text-green-800">Back to manufacturer portal</Link>
        ...
        <p className="mt-3 text-sm leading-6 text-neutral-700">
          Enter the email address connected to your approved manufacturer portal account. We will send a secure reset link if the email is registered.
        </p>
