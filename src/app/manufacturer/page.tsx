...
type OrdersPayload = {
  ok?: boolean;
  error?: string;
  summary?: { readyForFulfillment: number; echoBooked: number; readyToShip: number; shipped: number; total: number };
  orders?: Order[];
};
...
function Card({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-neutral-200"><p className="text-sm text-neutral-500">{label}</p><p className="mt-2 text-3xl font-black">{value}</p></div>;
}
...
export default function ManufacturerPortalPage() {
  ...
  useEffect(() => {
    async function start() {
      if (!supabase) {
        setError("Manufacturer auth is not available.");
        setLoading(false);
        return;
      }
      const { data } = await supabase.auth.getSession();
