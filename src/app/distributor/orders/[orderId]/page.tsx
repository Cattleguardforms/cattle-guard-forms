...
type DistributorOrder = { id: string; shortId: string; quantityLabel: string; quantity?: number; total: number; paymentStatus: string; checkoutStatus?: string; shipmentStatus?: string; status: string; shippingMethod?: string; selectedRate?: string; freightCharge?: number; carrier?: string; bolNumber?: string; trackingLink?: string; shipTo: string; createdAt?: string };
type OrderFile = { id: string; order_id: string; file_type: string; file_name: string; content_type?: string; size_bytes?: number; uploaded_by_role?: string; created_at?: string };
...
function money(value?: number) { return Number(value ?? 0).toLocaleString("en-US", { style: "currency", currency: "USD" }); }
function label(value?: string) { if (!value) return "Not available"; return value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
function dateLabel(value?: string) { if (!value) return "Not available"; const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleString(); }
function fileTypeLabel(value?: string) { if (!value) return "Order file"; return value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
function isBolFile(file: OrderFile) { const fileType = (file.file_type || "").toLowerCase(); const fileName = (file.file_name || "").toLowerCase(); return fileType.includes("bol") || fileName.includes("bol"); }
...
function DetailRow({ labelText, value }: { labelText: string; value?: string }) { return <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-neutral-500">{labelText}</p><p className="mt-1 text-sm font-bold text-neutral-950">{value || "Not available"}</p></div>; }
...
export default function DistributorOrderDetailPage() {
  ...
  async function getToken() { if (!supabase) throw new Error("Distributor auth unavailable."); const { data } = await supabase.auth.getSession(); const token = data.session?.access_token; if (!token) throw new Error("Distributor sign-in is required."); return token; }
  ...
  async function loadFiles(token: string) {
    ...
    if (!response.ok || !payload.ok) { setFileError(payload.error || "Unable to load order files."); setFiles([]); return; }
  ...
  async function fetchBol() {
    setFetchingBol(true); setNotice(""); setFileError("");
    try {
      const token = await getToken();
      ...
      if (first?.filename) setNotice(`Stored Echo BOL: ${first.filename}.`);
      else if (first?.reason === "bol_file_already_exists") setNotice("A BOL file is already stored for this order.");
      ...
      else setNotice("BOL recovery finished. Refresh files to check download status.");
      await loadFiles(token);
    } catch (err) { setFileError(err instanceof Error ? err.message : "Unable to fetch/store Echo BOL."); }
    finally { setFetchingBol(false); }
  }

  async function downloadFile(fileId: string) {
    setFileError(""); setDownloadingFileId(fileId);
    try { const token = await getToken(); const response = await fetch(`/api/order-files/download?fileId=${encodeURIComponent(fileId)}`, { headers: { Authorization: `Bearer ${token}` } }); const payload = (await response.json()) as DownloadPayload; if (!response.ok || !payload.ok || !payload.url) throw new Error(payload.error || "Unable to create download link."); window.open(payload.url, "_blank", "noopener,noreferrer"); }
    catch (err) { setFileError(err instanceof Error ? err.message : "Unable to download file."); }
  ...
  {notice ? <div className="mt-5 rounded border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-950">{notice}</div> : null}
  ...
  <section className="mt-6 rounded-2xl border border-neutral-200 bg-neutral-50 p-5"><h2 className="text-xl font-black text-green-950">Ship-To / Freight</h2><p className="mt-3 text-sm leading-6 text-neutral-700">{order.shipTo}</p><div className="mt-4 grid gap-3 md:grid-cols-3"><DetailRow labelText="Carrier" value={order.carrier} /><DetailRow labelText="BOL Number" value={order.bolNumber} /><DetailRow labelText="Tracking" value={order.trackingLink ? "Available" : "Not available"} /></div><div className="mt-4 flex flex-wrap gap-3">{order.trackingLink ? <Link href={order.trackingLink} className="rounded bg-blue-800 px-4 py-2 text-sm font-bold text-white hover:bg-blue-900">Open Tracking</Link> : null}<button type="button" onClick={() => void fetchBol()} disabled={fetchingBol || !canFetchBol(order)} className="rounded bg-blue-800 px-4 py-2 text-sm font-bold text-white hover:bg-blue-900 disabled:cursor-not-allowed disabled:opacity-50">{fetchingBol ? "Fetching / Storing BOL..." : "Fetch / Store Echo BOL"}</button><Link href={`/distributor/orders/${order.id}/warranty`} className="rounded bg-green-800 px-4 py-2 text-sm font-bold text-white hover:bg-green-900">Warranty Paperwork</Link><Link href="/distributor/documents" className="rounded border border-green-800 bg-white px-4 py-2 text-sm font-bold text-green-900 hover:bg-green-50">Documents / Packets</Link></div>{!canFetchBol(order) ? <p className="mt-3 text-xs font-semibold text-neutral-700">BOL recovery is enabled after Echo booking, Echo carrier data, or a BOL code exists.</p> : null}</section>
  <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-950"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="font-bold">BOL / Freight Document</p><p className="mt-2">BOL Code: <span className="font-mono font-bold text-neutral-950">{order.bolNumber || "Pending"}</span></p><p className="mt-2">If the BOL number exists but the file is missing, use Fetch / Store Echo BOL above. The download button appears here after the file is stored.</p></div><button type="button" onClick={() => void loadOrder()} className="rounded border border-amber-700 bg-white px-4 py-2 text-sm font-bold text-amber-950 hover:bg-amber-100">Refresh Files</button></div>{fileError ? <div className="mt-4 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{fileError}</div> : null}{bolFiles.length ? <div className="mt-4 grid gap-3">{bolFiles.map((file) => <div key={file.id} className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-white p-4 md:flex-row md:items-center md:justify-between"><div><p className="font-bold text-neutral-950">{file.file_name || "BOL document"}</p><p className="mt-1 text-xs text-neutral-600">{fileTypeLabel(file.file_type)}{file.created_at ? ` - ${dateLabel(file.created_at)}` : ""}</p></div><button type="button" onClick={() => void downloadFile(file.id)} disabled={downloadingFileId === file.id} className="rounded bg-green-800 px-4 py-2 text-sm font-bold text-white hover:bg-green-900 disabled:opacity-60">{downloadingFileId === file.id ? "Preparing..." : "Download BOL"}</button></div>)}</div> : <div className="mt-4 rounded-xl border border-amber-200 bg-white p-4"><p className="font-bold text-neutral-950">No downloadable BOL file is available yet.</p><p className="mt-1 text-sm text-neutral-700">Once the BOL document is fetched or uploaded, the Download BOL button will appear here.</p></div>}</section>
</section></main>;
}
