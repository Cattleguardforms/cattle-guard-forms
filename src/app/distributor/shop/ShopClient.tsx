...
export default function ShopClient() {
  ...
  const [distributorPoNumber, setDistributorPoNumber] = useState("");
  ...
  function resetQuote() { setQuoted(false); setRate(""); setFreight(0); }
  function handlePhoneChange(value: string) { setPhone(value); if (!customerPhone.trim()) setCustomerPhone(value); resetQuote(); }
  ...
  async function verify() {
    if (!supabase) { setError("Distributor auth unavailable."); setReady(true); return; }
    try {
  ...
  async function signIn(event: FormEvent<HTMLFormElement>) {
    ...
    try { if (!supabase) throw new Error("Distributor sign-in is not available."); const { error: signInError } = await supabase.auth.signInWithPassword({ email: loginEmail.trim().toLowerCase(), password }); if (signInError) throw new Error("Invalid distributor credentials."); setPassword(""); await verify(); }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!distributorPoNumber.trim()) return setError("Distributor PO number is required.");
    ...
    if (method === "echo" && (!rate || freight <= 0)) return setError("Select a freight quote before checkout.");
    ...
    try {
      ...
      fd.append("distributorPoNumber", distributorPoNumber);
