...
function getEchoFailureDetails(quote: FreightQuoteResponse | null) {
  ...
  return JSON.stringify({ status: quote.status, statusText: quote.statusText, echoResponse: quote.echoResponse }, null, 2);
}

export default function CustomerFreightQuotePanel({
  quantity,
  email,
  phone,
  shipToName,
...
}: CustomerFreightQuotePanelProps) {
  ...
  async function handleGetQuote() {
    ...
    if (!email.trim() || !email.includes("@") || !shipToName.trim() || !shipToAddress.trim() || !shipToCity.trim() || !shipToState.trim() || !shipToZip.trim()) {
      setError("Enter email, delivery phone, ship-to name, address, city, state, and ZIP before requesting freight.");
      return;
    }
  ...
  return (
    <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50 p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="font-black text-blue-950">Freight quote</p>
          <p className="mt-1 text-sm leading-6 text-blue-900">
            Get freight options, select one, then checkout unlocks.
          </p>
        </div>
        <button
          type="button"
          ...
          disabled={loading}
          className="rounded-xl bg-blue-800 px-5 py-3 font-black text-white hover:bg-blue-900 disabled:opacity-60"
        >
      ...
      {quote && !quote.ok ? (
        <details className="mt-4 rounded border border-red-200 bg-white p-4 text-sm text-red-900">
          ...
          <pre className="mt-3 max-h-72 overflow-auto rounded bg-neutral-950 p-3 text-xs text-neutral-50">{getEchoFailureDetails(quote)}</pre>
        </details>
      ) : null}
        ...
        <div className="mt-4 rounded border border-green-200 bg-white p-4 text-sm text-neutral-800">
          ...
          {bestRates.length ? (
              ...
              {bestRates.map((rate, index) => {
                ...
                return (
                  <label key={key} className="flex cursor-pointer gap-3 rounded border border-neutral-200 bg-neutral-50 p-3 hover:bg-white">
                    <input type="radio" name="customerFreightOption" checked={selectedRateKey === key} onChange={() => handleSelectRate(rate, index)} className="mt-1" />
            ...
            <div className="mt-4 rounded border border-amber-200 bg-amber-50 p-3 text-amber-900">
              Echo returned rates, but none passed the customer-facing filter. Please contact us for freight review.
            </div>
          )}
        </div>
      ) : null}
