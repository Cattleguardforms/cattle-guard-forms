...
function clampQuantity(quantity: number) {
  return Math.min(30, Math.max(1, quantity));
}
...
export default function QuotePage() {
  ...
  }, [formData.quantity, selectedFreightCharge]);
  ...
  const handleQuantityChange = (event: ChangeEvent<HTMLSelectElement>) => {
    setFormData((previous) => ({ ...previous, quantity: clampQuantity(Number.parseInt(event.target.value, 10)) }));
    resetFreightSelection();
  };

  const handleDeliverySelectChange = (event: ChangeEvent<HTMLSelectElement>) => {
    const { name, value } = event.target;
    setFormData((previous) => ({ ...previous, [name]: value }));
    resetFreightSelection();
  };
  ...
  function validate() {
    ...
    if (!formData.shipToName || !formData.shipToAddress || !formData.shipToCity || !formData.shipToState || !formData.shipToZip) {
      return "Please enter the full delivery address.";
    }
    if (!hasFreightQuote || !selectedFreightRate || selectedFreightCharge <= 0) {
      return "Select a freight option before payment.";
    }
    return null;
  }
    ...
    try {
      const response = await fetch("/api/customer-checkout", {
        ...
        body: JSON.stringify({
          ...
          shipToAddress2: formData.shipToAddress2,
          shipToCity: formData.shipToCity,
          shipToState: formData.shipToState,
          shipToZip: formData.shipToZip,
      ...
      <section className="relative overflow-hidden bg-green-950 text-white">
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:py-20">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-green-50">Enter delivery details, select freight, and checkout securely.</p>
          </div>
        ...
        <section className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <aside className="space-y-6">
            ...
            <div className="rounded-3xl border border-green-100 bg-green-50 p-6 shadow-sm">
              ...
              <p className="mt-4 text-sm leading-6 text-neutral-700">Grooves are {COWSTOP_SPECS.grooveDepth} deep.</p>
            </div>
          ...
          <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-xl">
            <div className="rounded-2xl bg-neutral-50 p-5">
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <select name="quantity" value={formData.quantity} onChange={handleQuantityChange} className="rounded-xl border border-neutral-300 bg-white px-6 py-3 text-center font-bold outline-none">
                  {Array.from({ length: 30 }, (_, index) => index + 1).map((quantity) => <option key={quantity} value={quantity}>{quantity}</option>)}
                </select>
              </div>
              <dl className="mt-6 space-y-3 text-sm">
                <div className="flex justify-between"><dt className="text-neutral-500">Product subtotal</dt><dd className="font-bold">{currencyFormatter.format(pricing.subtotal)}</dd></div>
                <div className="border-t border-neutral-200 pt-3"><div className="flex justify-between"><dt className="text-base font-black">Product total</dt><dd className="text-2xl font-black text-green-950">{currencyFormatter.format(pricing.productTotal)}</dd></div></div>
                {selectedFreightCharge > 0 ? <div className="flex justify-between"><dt className="text-neutral-500">Freight & handling</dt><dd className="font-bold">{currencyFormatter.format(selectedFreightCharge)}</dd></div> : null}
                <div className="border-t border-neutral-200 pt-3"><div className="flex justify-between"><dt className="text-base font-black">Total due today</dt><dd className="text-3xl font-black text-green-950">{currencyFormatter.format(pricing.totalDueToday)}</dd></div></div>
              </dl>
            </div>
            ...
            <div className="mt-6 grid gap-6">
              <section>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <input required name="email" type="email" value={formData.email} onChange={handleChange} placeholder="Order contact email *" className="rounded-xl border border-neutral-300 px-3 py-3 sm:col-span-2" />
                  <input name="firstName" value={formData.firstName} onChange={handleChange} placeholder="First name" className="rounded-xl border border-neutral-300 px-3 py-3" />
                  <input name="lastName" value={formData.lastName} onChange={handleChange} placeholder="Last name" className="rounded-xl border border-neutral-300 px-3 py-3" />
                  <input required name="phone" type="tel" value={formData.phone} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="Delivery contact phone *" className="rounded-xl border border-neutral-300 px-3 py-3" />
              ...
              <section>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <input required name="shipToName" value={formData.shipToName} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="Ship-to name / company *" className="rounded-xl border border-neutral-300 px-3 py-3 sm:col-span-2" />
                  <input required name="shipToAddress" value={formData.shipToAddress} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="Delivery address line 1 *" className="rounded-xl border border-neutral-300 px-3 py-3 sm:col-span-2" />
                  <input name="shipToAddress2" value={formData.shipToAddress2} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="Delivery address line 2" className="rounded-xl border border-neutral-300 px-3 py-3 sm:col-span-2" />
                  <input required name="shipToCity" value={formData.shipToCity} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="City *" className="rounded-xl border border-neutral-300 px-3 py-3" />
                  <input required name="shipToState" value={formData.shipToState} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="State *" className="rounded-xl border border-neutral-300 px-3 py-3" />
                  <input required name="shipToZip" value={formData.shipToZip} onChange={(event) => { handleChange(event); resetFreightSelection(); }} placeholder="ZIP *" className="rounded-xl border border-neutral-300 px-3 py-3 sm:col-span-2" />
                </div>
              </section>

              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
                <h2 className="text-xl font-black text-amber-950">Delivery Details</h2>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-2 text-sm font-bold text-amber-950">Delivery location type
                    <select required name="deliveryType" value={formData.deliveryType} onChange={handleDeliverySelectChange} className="rounded-xl border border-amber-200 bg-white px-3 py-3 font-normal text-neutral-950">
                      <option value="residential">Residential / farm / home</option>
                      <option value="commercial">Commercial / business</option>
                    </select>
                  </label>
                  <label className="grid gap-2 text-sm font-bold text-amber-950">Liftgate required?
                    <select required name="liftgateRequired" value={formData.liftgateRequired} onChange={handleDeliverySelectChange} className="rounded-xl border border-amber-200 bg-white px-3 py-3 font-normal text-neutral-950">
            ...
            <button type="submit" disabled={checkoutLoading || !hasFreightQuote} className="mt-8 w-full rounded-xl bg-green-800 px-5 py-4 font-black text-white shadow-lg shadow-green-950/20 hover:bg-green-900 disabled:cursor-not-allowed disabled:opacity-50">
